...
concat (Seq f) = (foldr Jhc.Util.Seq.append Jhc.Util.Seq.empty (f []))

instance Functor Jhc.Util.Seq.Seq where
    --fmap f xs = Seq.fromList (map f (Seq.toList xs))
    fmap f (Seq xs) = Seq (\ts -> map f (xs []) ++ ts )

instance Monad Jhc.Util.Seq.Seq where
    --a >>= b  = mconcat ( fmap b (Seq.toList a))
    a >>= b  = Jhc.Util.Seq.concat (fmap b a)
    return x = Jhc.Util.Seq.single x
    fail _ = Jhc.Util.Seq.empty

instance MonadPlus Jhc.Util.Seq.Seq where
    mplus = mappend
    mzero = Jhc.Util.Seq.empty
