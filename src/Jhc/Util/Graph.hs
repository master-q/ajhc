...
module Jhc.Util.Graph(
    ...
    sccForest,
    Jhc.Util.Graph.dff,
    Jhc.Util.Graph.components,
    Jhc.Util.Graph.topSort,
    cyclicNodes,
    toDag,
    restitchGraph,
    mapGraph,
...
instance Show n => Show (Graph n) where
    showsPrec n g = showsPrec n (Jhc.Util.Graph.scc g)
...
newGraphReachable ns fn fd = (rable,ng) where
    ...
    rable ks = Jhc.Util.Graph.reachable ng [ v | Just v <- map (flip Map.lookup vmap) ks ]
...
sccGroups :: Graph n -> [[n]]
sccGroups g = map fromScc (Jhc.Util.Graph.scc g)
...
cyclicNodes g = concat [ xs | Right xs <- Jhc.Util.Graph.scc g]
