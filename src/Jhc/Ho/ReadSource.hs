...
import Jhc.FrontEnd.HsParser
import Jhc.FrontEnd.HsSyn
import Jhc.FrontEnd.ParseMonad
import Jhc.FrontEnd.SrcLoc
import Jhc.FrontEnd.Syn.Options
import Jhc.FrontEnd.Unlit
import Jhc.FrontEnd.Warning
import Jhc.Options
import Jhc.PackedString
import Jhc.RawFiles(prelude_m4)
import Jhc.Support.TempDir
import Jhc.Util.FilterInput
import Jhc.Util.Gen
import Jhc.Version.Config(revision,version)
...
preprocess opt fn lbs = do
    ...
    case () of
        _ | fopts FO.Cpp -> readSystem "cpphs" $ incFlags ++ defFlags ++ [fn]
          | fopts FO.M4  -> do
                m4p <- m4Prelude
                readSystem "m4" $ ["-g", "-s", "-P"] ++ incFlags ++ defFlags ++ [m4p,fn]
