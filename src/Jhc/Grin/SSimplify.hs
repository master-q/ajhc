...
module Jhc.Grin.SSimplify(simplify,explicitRecurse) where
...
instance Jhc.Stats.MonadStats S where
    mtickStat s = S (tell mempty { colStats = s })
    mticks' n a = S (tell mempty { colStats = Jhc.Stats.singleStat n a })
