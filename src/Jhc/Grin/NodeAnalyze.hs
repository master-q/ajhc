...
module Jhc.Grin.NodeAnalyze(nodeAnalyze) where
...
nodeAnalyze grin' = do
    ...
    (grin',stats) <- Jhc.Stats.runStatT $ tickleM (fixupfs cmap (grinTypeEnv grin)) grin
...
doFunc (name,arg :-> body) = ans where
    ...
    equals x y | isGood x && isGood y = Jhc.Util.UnionSolve.equals x y
               | otherwise = mempty
    isgte x y | isGood x && isGood y = Jhc.Util.UnionSolve.isgte x y
              | otherwise = mempty
    islte x y | isGood x && isGood y = Jhc.Util.UnionSolve.islte x y
              | otherwise = mempty
...
fixupfs cmap tyEnv l = tickleM f (l::Lam) where
    ...
    f a@(BaseOp Eval [arg]) | Just n <- lupVar arg = case n of
        N WHNF _ -> do
            pstuff "eval" arg n
            Jhc.Stats.mtick (toAtom "Optimize.NodeAnalyze.eval-promote")
    ...
    f a@(BaseOp (Apply ty) (papp:args)) | Just nn <- lupVar papp = case nn of
        N WHNF tset | Only set <- tset, [sv] <- Set.toList set, TagPApp n fn <- tagInfo sv, Just (ts,_) <- findArgsType tyEnv sv -> do
            ...
            case (n,args) of
                (1,[arg]) -> do
                    Jhc.Stats.mtick (toAtom "Optimize.NodeAnalyze.apply-inline")
