...
module Jhc.C.FromGrin2(compileGrin) where
...
runC grin (C m) =  (execUniq1 (runRWST m startEnv emptyHcHash),ityrep) where
    ...
    ityrep = Map.mapMaybeWithKey tyRep (fromDistinctAscList $ SL.toList tmap)
    startEnv = Env {
        rCPR = ityrep,
        rGrin = grin,
        rStowed = Set.empty,
