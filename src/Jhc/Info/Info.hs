module Jhc.Info.Info(
    T,
    Info(..),
    Entry(..),
    HasInfo(..),
    Jhc.Info.Info.lookup,
    Jhc.Info.Info.lookupTyp,
    insertWith,
    insert,
    limit,
    maybeInsert,
...
infoMapM f i = case Jhc.Info.Info.lookup i of
    Just x -> do
        n <- f x
...
infoMap f i = case Jhc.Info.Info.lookup i of
...
fetch info = maybe mempty id  (Jhc.Info.Info.lookup info)
