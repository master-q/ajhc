...
import Jhc.Doc.DocLike hiding(empty)
import qualified Jhc.Doc.DocLike as DocLike
...
fillSep         = fold (</>)
--hsep            = fold (<+>)
vsep            = fold (Jhc.Doc.Pretty.<$>)

cat             = group . Jhc.Doc.Pretty.vcat
fillCat         = fold (<//>)
--hcat            = fold (<>)
vcat            = fold (<$$>)
...
instance Monoid Doc where
    mempty = Jhc.Doc.Pretty.empty
    mappend = beside
    mconcat = fold beside

instance TextLike Doc where
    empty = Jhc.Doc.Pretty.empty
    text = mytext
    char x = mychar x
...
instance DocLike Doc where
    x <> y = x `beside` y
    x <+> y = x <> space <> y
    encloseSep = Jhc.Doc.Pretty.encloseSep
    vcat = Jhc.Doc.Pretty.vcat
    --hcat = Jhc.Doc.Pretty.hcat
    --hsep = Jhc.Doc.Pretty.hsep
