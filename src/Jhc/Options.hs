...
import Jhc.RawFiles(targets_ini)
import Jhc.Support.IniParse
import Jhc.Support.TempDir
import Jhc.Support.Cabal
import Jhc.Util.ExitCodes
import Jhc.Util.Gen
import Jhc.Util.YAML
import Jhc.Version.Config
import Jhc.Version.Version(versionString,versionContext)
import qualified Jhc.FlagDump
import qualified Jhc.FlagOpts
import qualified Jhc.Version.Config as VC
...
data Opt = Opt {
    ...
    optDumpSet     ::  S.Set Jhc.FlagDump.Flag,    -- ^ Dump flags.
    optFOptsSet    ::  S.Set Jhc.FlagOpts.Flag     -- ^ Flag options (-f\<opt\>).
  }
...
emptyOpt = Opt {
    ...
    optStatLevel   = 1,
    optNoAuto      = False,
    optDumpSet     = S.singleton Jhc.FlagDump.Progress,
    optFOptsSet    = S.empty
}
...
postProcessFD o = case Jhc.FlagDump.process (optDumpSet o) (optDump o ++ vv) of
        (s,[]) -> return $ o { optDumpSet = s, optDump = [] }
        (_,xs) -> fail ("Unrecognized dump flag passed to '-d': "
                        ++ unwords xs ++ "\nValid dump flags:\n\n" ++ Jhc.FlagDump.helpMsg)
    where
...
postProcessFO o = case Jhc.FlagOpts.process (optFOptsSet o) (optFOpts o) of
        (s,[]) -> return $ o { optFOptsSet = s, optFOpts = [] }
        (_,xs) -> fail ("Unrecognized flag passed to '-f': "
                        ++ unwords xs ++ "\nValid flags:\n\n" ++ Jhc.FlagOpts.helpMsg)
...
helpUsage = usageInfo header theoptions ++ trailer where
    ...
    trailer = "\n" ++ mkoptlist "-d" Jhc.FlagDump.helpFlags ++ "\n" ++ mkoptlist "-f" Jhc.FlagOpts.helpFlags
...
processOptions = do
    ...
    when (Jhc.FlagDump.Ini `S.member` optDumpSet o) $ flip mapM_ (M.toList inis) $ \(a,b) -> putStrLn (a ++ "=" ++ b)
    -- set flags based on ini options
    let o1 = case M.lookup "gc" inis of
            Just "jgc" -> optFOptsSet_u (S.insert Jhc.FlagOpts.Jgc) o
            Just "boehm" -> optFOptsSet_u (S.insert Jhc.FlagOpts.Boehm) o
            _ -> o
    o2 <- either putErrDie return $ postProcessFO o1
    when (Jhc.FlagDump.Ini `S.member` optDumpSet o) $ do
...
-- | Is verbose > 0?
progress :: Bool
progress = dump Jhc.FlagDump.Progress
...
flagOpt :: OptionMonad m => Jhc.FlagOpts.Flag -> m Bool
flagOpt flag = do
    opt <- getOptions
